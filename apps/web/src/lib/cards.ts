...
export const CARDS: CardInfo[] = [
  ...
  { id: 26000037, name: "Wall Breakers", elixir: 3, rarity: "epic", type: "troop" },
  ...
  { id: 26000045, name: "E-Golem", elixir: 4, rarity: "epic", type: "troop" },
  { id: 26000046, name: "Skeleton Dragon", elixir: 4, rarity: "epic", type: "troop" },
  ...
  { id: 26000093, name: "Snowball", elixir: 2, rarity: "common", type: "spell" },
  { id: 26000094, name: "Log", elixir: 2, rarity: "common", type: "spell" },
...
export function findCard(name: string): CardInfo | undefined {
  const normalize = (s: string) =>
    s.toLowerCase().replace(/\./g, "").replace(/ evolution$/i, "");
...
export function stripEvolution(name: string): string {
  // Remove any trailing " evolution", " evolved" or " (Hero)" designations
  return name.replace(/\s*(?:\(?.*?(?:evolution|evolved|hero)\)?)?$/i, "").trim();
}
...
export function deduplicateCards(
  ...
  for (const c of cards) {
    ...
    if (!existing || evolved || ratio > existing.ratio) {
